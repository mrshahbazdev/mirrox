  ...
  return (
    <div className="min-h-screen bg-[#050507] text-slate-300 flex flex-col font-sans overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          {variations.length > 0 && (
             <button onClick={() => { navigator.clipboard.writeText(currentHTML); setCopied(true); setTimeout(()=>setCopied(false),2000); }} 
              className={`px-5 py-2 rounded-xl text-[10px] uppercase font-black tracking-widest transition-all flex items-center gap-2 ${copied ? 'bg-emerald-500 text-white' : 'bg-white text-black hover:bg-slate-200'}`}>
              {copied ? <Check size={14}/> : <Share2 size={14}/>} {copied ? 'Project Synced' : `Copy ${techTarget.toUpperCase()}`}
            </button>
          )}
          <button onClick={() => setShowSettings(true)} className="w-10 h-10 rounded-xl hover:bg-white/5 flex items-center justify-center text-slate-500 border border-transparent hover:border-white/5 transition-all">
            <Settings size={20} />
          </button>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden relative z-10">
        ...
        {/* SIDEBAR */}
        <aside className="w-[340px] bg-[#0a0a0c] border-r border-white/5 flex flex-col no-scrollbar overflow-y-auto">
          <div className="p-6 flex flex-col gap-6">
            
            {/* Input Toggle */}
            <div className="bg-white/5 p-1 rounded-2xl border border-white/5 grid grid-cols-2 gap-1 mb-2">
               <button onClick={() => setInputMode('prompt')} className={`py-2 rounded-xl text-[10px] font-black uppercase transition-all ${inputMode === 'prompt' ? 'bg-white/10 text-white shadow-xl' : 'text-slate-500 hover:text-slate-300'}`}>Prompt</button>
               <button onClick={() => setInputMode('convert')} className={`py-2 rounded-xl text-[10px] font-black uppercase transition-all ${inputMode === 'convert' ? 'bg-white/10 text-white shadow-xl' : 'text-slate-500 hover:text-slate-300'}`}>Vision</button>
            </div>
            ...
            {variations.length > 0 && (
              <div className="mt-2 pt-6 border-t border-white/5 space-y-6">
                 ...
                 <div className="bg-white/5 p-5 rounded-3xl border border-white/5 space-y-5">
                    <div>
                      <div className="flex items-center justify-between text-[10px] font-black text-slate-500 uppercase mb-3">Accent <span>{liveStyles.primary}</span></div>
        ...
        {/* VIEWPORT */}
        <section className="flex-1 flex flex-col relative bg-[#0b0b0e]">
          <div className="h-16 border-b border-white/[0.04] flex items-center justify-between px-8 bg-black/40 backdrop-blur-md">
             <div className="flex items-center gap-6">
                <div className="flex bg-white/5 p-1 rounded-2xl border border-white/5">
                  {['preview', 'code', 'history'].map(t => (
                    <button key={t} onClick={() => setActiveTab(t)} className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === t ? 'bg-white/10 text-white shadow-xl' : 'text-slate-500 hover:text-slate-300'}`}>{t}</button>
                  ))}
                </div>
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-8">
           <div className="absolute inset-0 bg-black/90 backdrop-blur-2xl" onClick={() => setShowSettings(false)} />
                ...
                <div className="space-y-5">
                   ...
                   <div className="grid grid-cols-2 gap-4">
                     <div>
                       ...
                       <input type="color" value={brand.primary} onChange={e=>setBrand({...brand, primary: e.target.value})} className="w-full h-12 rounded-2xl bg-white/5 border border-white/5 p-1 cursor-pointer" />
                     </div>
                     <div>
                       <Label>Radius</Label>
                       <select value={liveStyles.radius} onChange={e=>setLiveStyles({...liveStyles, radius: e.target.value})} className="w-full bg-white/5 border border-white/5 rounded-2xl p-3.5 text-xs text-white outline-none appearance-none font-bold">
                   ...
                   <div>
                     ...
                     <input type="text" value={brand.logo} onChange={e=>setBrand({...brand, logo: e.target.value})} className="w-full bg-white/5 border border-white/5 rounded-2xl p-4 text-xs text-white outline-none" placeholder="https://..." />
                   </div>
                   <div>
                     <Label>Typography Base</Label>
                     <select value={brand.font} onChange={e=>setBrand({...brand, font: e.target.value})} className="w-full bg-white/5 border border-white/5 rounded-2xl p-4 text-xs text-white outline-none appearance-none font-bold">
